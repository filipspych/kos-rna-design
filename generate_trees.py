...
def generate_tree(n: int) -> Graph:
    ...
    n -= root_unpaired
    print(n)
    child_amount = root_paired * 2 + root_unpaired
    ...
    while len(stack) > 0:
        ...
        if n <= 0:
            continue
        node_paired = random.randint(0, min(3, n/2))
        node_unpaired = 0
        if node_paired == 0 or node_paired == 1:
            node_unpaired = random.randint(0, n - node_paired * 2)

        child_amount = node_paired * 2 + node_unpaired
        n -= child_amount
        print(n)
        pair_indexes = random.sample(range(1, child_amount + 1), node_paired)
